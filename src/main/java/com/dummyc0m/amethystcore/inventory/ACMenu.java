...
public class ACMenu {
    ...
    private ACAbstractButton[] contents;

    public ACMenu(int size, String title, ACAbstractButton[] contents) {
        this(size, title);
    ...
    public ItemStack onClick(Inventory inventory, InventoryType.SlotType type, int slot, ClickType click, ItemStack item, HumanEntity whoClicked){
        ACAbstractButton button = this.contents[slot];
        if(button != null){
            switch(click){
                case LEFT:
    ...
    public void display(Player player){
        ...
        int i = 0;
        for (ACAbstractButton b : contents) {
    ...
    public ACAbstractButton[] getContents() {
        return contents;
    }

    public void setContents(ACAbstractButton[] items) {
        this.contents = items;
    }
